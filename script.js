...
veryOldInventors = inventors.filter((num) => {
    return num.year >= 1500 && num.year <= 1599
}); 
...
inventorNamedAda =  inventors.find((nameFound) => {
    return nameFound.first === 'Ada';
});
...
isAdultPresent = devs.some((ofAge) => {
    let currentYear = new Date().getFullYear();
    let ages = currentYear - ofAge.year
    return ages >= 18;
});
...
isEveryone19OrOlder = devs.every((peopleofAge) => {
    let currentYear = new Date().getFullYear();
    let age = currentYear - peopleofAge.year;
    return age >= 19;
});
...
idx = comments.findIndex((item) => {
    return item.id === 123523
});
